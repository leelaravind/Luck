...
export function buildSystemPrompt(obs: GameObservation, opts: { allowStop?: boolean } = {}): string {
  ...
  return [
    ...
    `- every stake is an integer >= ${l.minStake} and a multiple of ${l.stakeIncrement}${l.maxStakePerBet === null ? '' : `, and <= ${l.maxStakePerBet}`}`,
    l.maxStakePerRound === null
      ? '- the combined stake of all bets in a round can be anything up to your current balance'
      : `- the combined stake of all bets in a round is <= ${l.maxStakePerRound} and <= your current balance`,
    l.maxBetsPerRound === null ? '- there is no limit on the number of bets per round' : `- at most ${l.maxBetsPerRound} bets per round`,
    `- ${rounds}`,
    '',
    'OBJECTIVE',
