...
#
# Works from any folder. Checks Node.js (22.22.2 or newer 22.x, or 24.15.0 or newer), runs
...
NODE_REQUIREMENT="Node.js 22 LTS (22.22.2 or newer) or Node.js 24.15.0 or newer"
...
if ! node -e '
  ...
  const ok = (a === 22 && (b > 22 || (b === 22 && c >= 2))) || (a === 24 && b >= 15) || a > 24;
  process.exit(ok ? 0 : 1);
'; then
  fail "Node.js $NODE_VERSION is not supported. Luck needs $NODE_REQUIREMENT (the oldest releases it is tested with). Download it from https://nodejs.org/."
fi
